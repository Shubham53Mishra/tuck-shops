    ...
    if (empty($name) || empty($email) || empty($phone) || empty($city) || empty($message)) {
        ...
        if ($conn->query($sql) === TRUE) {
            ...
            try {
                ...
                $mail->Username = 'munchmart2024@gmail.com'; // Your Gmail address
                $mail->Password = 'drzk wzqj lgra vsdl'; // Use your Gmail app-specific password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // Enable TLS encryption
                $mail->Port = 587; // TCP port to connect to

                // Recipients
                $mail->setFrom('munchmart2024@gmail.com', 'Munch Mart!');
